...
**Phase 3 — MCP server alongside the repo — has started.** Tier-1 ships an `[mcp]`-extra `cairn mcp` subprocess (stdio transport, FastMCP) plus a single user-level registry (`~/.config/cairn/server.toml`) so one MCP server serves multiple cairns (ADR-0010). Tools so far: `whoami`, `status`, `get_open_questions`, `get_action_items`, `add_decision`, `add_finding`, `add_action`, `complete_action` — all per ADR-0009's Tier-1 list. New CLI commands: `cairn register`, `cairn unregister`, `cairn registered`, `cairn link`, `cairn mcp`. HTTP transport (`--transport streamable-http|sse`), remote pairing, and CLI remote dispatch landed via US-P-11/12/13 (ADR-0012). Server-side bearer-token auth landed via US-P-14 (ADR-0013): `cairn token issue|revoke|list` manages a hashed token store at `~/.config/cairn/server_tokens.toml`, and `cairn mcp --auth token` gates HTTP requests on a valid bearer. Token → `author` binding is explicitly deferred. Tier 2 (explorations, findings reads, search) and Tier 3 (full read surface) follow once UX testing of Tier 1 surfaces what's missing.
